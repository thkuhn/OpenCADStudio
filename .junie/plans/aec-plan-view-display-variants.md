...
### ✓ Step 9: Stil-/Material-Kopie zwischen Projekt- und Standard-Bibliothek
Stile (Wandstile und Materialien) können gezielt und manuell von der Projekt-Bibliothek in die Standard-(globale) Bibliothek kopiert werden und umgekehrt, jeweils mit optionalem Überschreiben eines bereits vorhandenen Zieleintrags.
...
- Konflikt-/Überschreiben-Dialog bei Namens-/ID-Kollision im Zielbestand (Kopieren wird nur nach expliziter Bestätigung überschrieben, sonst abgebrochen/umbenannt).
- Baut auf bereits vorhandenen Bausteinen auf: `resolve_style_library(project)` (Auflösung Projekt vs. Standard) und `unique_id(prefix, name)` (Kollisionsvermeidung bei neuen IDs).
- Explizit **kein** Auto-Sync/automatischer Abgleich zwischen den Bibliotheken — nur explizites, manuelles Kopieren ist Teil dieser späteren Stufe.
...
- ✓ Bugfix: Wandstil-/Material-IDs waren rein namensbasiert (`format!("style_{}"/"mat_{}", slugify(name))`), wodurch zwei unabhängig angelegte Stile mit gleichem Namen in unterschiedlichen Projekten dieselbe ID erhielten und beim Mischen (z. B. Wand aus anderem Projekt) die falschen Schichtdaten "erbten". Fix: neue Einträge erhalten über `commands::unique_id(prefix, name)` zusätzlich ein zufälliges UUID-Suffix (`style_<slug>_<8-hex>`); bestehende IDs beim Bearbeiten bleiben unverändert. Betrifft `src/ui/window/aec_style_manager.rs`-Speicherpfad (`app/update/mod.rs`) sowie die `AEC_STYLE_ADD`/`AEC_MATERIAL_ADD`-Kommandos (`commands.rs`).
- ✓ Bugfix: Beim Zuweisen eines Wandstils an eine Wand (Style-Picker) sowie beim Speichern+Anwenden eines Wandstils im AEC-Style-Manager wurde die Wand mit `regenerate_wall_representation` ohne `ComponentRuleSet`/`style_substitutions` neu gezeichnet, wodurch die aktive `DisplayConfig` (Planart) der Zeichnung ignoriert wurde und die Wand stattdessen immer im Standard-LOD (alle Slots sichtbar, Original-Stil) erschien. Fix: neue Helferfunktion `App::regenerate_wall_respecting_active_display_config(tab_index, wall_handle)` (`src/app/update/mod.rs`) löst `tabs[tab_index].active_display_config` gegen `aec_plan_library` auf und ruft bei aktiver Planart `regenerate_wall_representation_with_rules_and_substitutions` statt der Standardfunktion auf (mirrort `apply_display_config_to_scene`); beide betroffenen Aufrufstellen nutzen jetzt diese Helferfunktion.
- ✓ Bugfix: `regenerate_wall_representation_inner` lud die Material-/Wandstil-Bibliothek immer projekt-unaware über `load_or_seed()` statt über `resolve_style_library(project)`, wodurch Schraffur-Skalierung/-Muster/-Farbe (und alle anderen materialgesteuerten Wanddarstellungen) bei geöffnetem Projekt still die globale Default-Bibliothek statt der projektgebundenen Materialien nutzten. Fix: optionales `library_override: Option<&StyleLibrary>` wird durch die Wand-Regenerierungs-Aufrufkette gereicht; echte (Nicht-Test-)Aufrufer mit Projektzugriff lösen die Bibliothek einmalig via `resolve_style_library` auf und übergeben `Some(&library)`, Tests übergeben `None` und behalten damit exakt das bisherige `load_or_seed()`-Verhalten.
- ✓ Bugfix: Die Statusleisten-Pille "Planart" blieb leer, weil `App::aec_plan_library` nur beim Öffnen des Plan-Managers (bzw. via Auto-Maßstabskopplung) befüllt wurde. Fix: `aec_plan_library` wird jetzt zusätzlich beim Öffnen einer Zeichnung (`Message::FileOpened`, `src/app/update/file.rs`) sowie beim Neuanlegen/Laden eines Projekts (`AecProjectExplorerNew`/`AecProjectExplorerLoadResult`, `src/app/update/mod.rs`) eager über `resolve_display_config_library` aufgelöst, sodass die Auswahl sofort vollständig ist.
- ✓ Bugfix: Wände verbinden/verlängern/umkehren sowie Öffnungen setzen (`aec_walljoin_do`, `aec_wallextend_do`, `aec_wallreverse_do`, `aec_wallopening_do`, `join_two_walls_in_document`, `join_junction_in_document`, `try_auto_join_nearby_walls` u.a. in `commands.rs`) regenerierten die betroffenen Wände bisher immer ohne `ComponentRuleSet`/`style_substitutions`, wodurch die aktive Planart bei diesen Bearbeitungen ignoriert wurde (analog zum bereits behobenen Style-Zuweisungs-Bug). Fix: Alle Funktionen dieser Aufrufkette erhalten zusätzlich `display_rules: Option<&ComponentRuleSet>` und `style_substitutions: Option<&HashMap<WallStyleRef, WallStyleRef>>` und reichen sie an die `_with_rules_and_substitutions`-Varianten der Wandregenerierung durch; ein neuer gemeinsamer Helfer `App::resolve_active_display_config_wall_rules(tab_index)` (`src/app/update/mod.rs`) löst die aktive `DisplayConfig` einmalig auf und wird von allen Produktions-Aufrufstellen (u. a. `src/app/commands/draw.rs`, `src/app/command_driver.rs`, `src/app/properties.rs`, `src/app/update/viewport.rs`) sowie von `regenerate_wall_respecting_active_display_config` selbst genutzt; Test-Aufrufstellen übergeben `None, None` und bleiben unverändert.
- ⚠ Untersuchung inkonklusiv (Wandschraffur-Skalierung beim Laden): `HatchPattern.scale` (`src/scene/model/hatch_model.rs`) wird sowohl beim Bake in `regenerate_wall_representation_inner` als auch beim Rendern ausschließlich aus dem festen `material.hatch_scale`-Wert abgeleitet (mit Floor 0.01) — an keiner Stelle (weder `commands.rs` noch `hatch_model.rs`/`entity.rs`) besteht eine Abhängigkeit von `annotation_scale` oder einem sonstigen, beim Laden neu berechneten Skalierungswert. Damit sind beide bisherigen Hypothesen (render-time-Skalierung sowie bake-time-Kopplung an `annotation_scale`) widerlegt; der ursprünglich berichtete Effekt konnte code-seitig keiner konkreten Ursache zugeordnet werden. Kein Fix vorgenommen, um keine spekulative Änderung ohne bestätigte Root Cause einzuführen. Nächster Schritt bei Bedarf: manuelle Reproduktion mit dem Nutzer (Wand mit klar erkennbarem Schraffurmuster speichern, Anwendung neu starten, Datei erneut öffnen, exakt beschreiben was optisch abweicht).
- Feature "Stile von Projekt- zu Standard-Bibliothek und umgekehrt kopieren, optional überschreiben" wurde besprochen und ist als spätere Ausbaustufe (siehe Step 9) festgehalten, nicht Teil der aktuellen Umsetzung.
- ✓ Feature umgesetzt (Step 9): Kopieren von Wandstilen und Materialien zwischen Projekt- und Standard-Bibliothek ist jetzt in beide Richtungen möglich, mit expliziter Überschreiben-Bestätigung bei Namens-/ID-Kollision. Konflikterkennung über `CopyConflict`/`material_copy_conflict`/`wall_style_copy_conflict` (`src/modules/aec/engine/library.rs`); Auslösung über die neuen `Message`-Varianten `AecStyleManagerCopyMaterialToProject`/`AecStyleManagerCopyMaterialToGlobal`/`AecStyleManagerCopyWallStyleToProject`/`AecStyleManagerCopyWallStyleToGlobal` sowie `AecStyleManagerCopyConflictConfirm` (`src/app/mod.rs`), verarbeitet in `src/app/update/mod.rs`; UI-Buttons "→ Standard"/"→ Projekt" in `aec_material_manager.rs`/`aec_wall_style_manager.rs`, Bestätigungsdialog `ModalKind::AecStyleCopyConflict` in `src/app/view/modal.rs`.
